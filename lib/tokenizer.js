...
Tokenizer.prototype.tryScanTag = function tryScanTag() {
  ...
  if(this.str[this.pos + 1] !== '/' && !this.str[this.pos + 1].match(/\w/)) return;
    ...
    if(this._newline) {
      ...
      if(str[this.pos].match(/\d/)) {
        ...
        if(str[this.pos] === '.' && str[this.pos + 1] === ' ') {
          ...
          if(ws.length > 0) this.enqueue({ name: 'whitespace', contents: ws });
...
Tokenizer.prototype.enqueue = function enqueue(tok) {
  ...
  if(this._lookahead.length === 0) return;
...
Tokenizer.prototype.next = function next() {
  ...
  if(this.previous.name !== 'EOF') this.dequeue();
