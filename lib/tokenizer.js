...
module.exports = class Tokenizer {
  ...
  scanDigits() {
    ...
    while (this.str[this.pos].match(Re.digit)) {
      this.pos++;
    }
  ...
  tryScanTag() {
    ...
    const match = this.str.slice(this.pos).match(Re.tag);
    if (!match) {
      return;
    }
  ...
  tryScanComment() {
    const match = this.str.slice(this.pos).match(Re.comment);
    if (!match) {
      return;
    }
