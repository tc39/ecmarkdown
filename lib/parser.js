...
class Parser {
  ...
  parseSegment(inList, fmtStack) {
    ...
    while (isSegmentNode(this._t.previous, this._t.peek(), inList, fmtStack)) {
      ...
      if (tok.name === 'chars' || tok.name === 'whitespace' || tok.name === 'linebreak') {
        seg.push(this.parseChars(inList, fmtStack));
  ...
  parseChars(inList, fmtStack) {
    const node = { name: 'chars', contents: '' };
    let tok = this._t.peek();

    while (tok.name !== 'EOF' &&
          (!inList || tok.name !== 'list') &&
          (isChars(tok) || isWhitespace(tok) || !isValidFormat(this._t.previous, tok, this._t.peek(2), fmtStack))
    ) {
      this._t.next();
...
module.exports = function parse(emd) {
