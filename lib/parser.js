...
module.exports = class Parser {
  ...
  parseFragment(opts, fmtStack) {
    ...
    while (true) {
      ...
      if (tok.name === 'opaqueTag' || tok.name === 'blockTag' ||
        ...
        break;
      } else if (tok.name === 'parabreak' && !opts.allowParabreak) {
        break;
      } else if (tok.name === 'text' || tok.name === 'whitespace' ||
                 tok.name === 'linebreak' || tok.name === 'parabreak') {
  ...
  parseText(opts, fmtStack) {
    ...
    while (true) {
      ...
      let wsChunk = '';
      while (isWhitespace(tok) || (opts.allowParabreak && tok.name === 'parabreak')) {
      ...
      if (tok.name === 'EOF' || tok.name === 'parabreak' || tok.name === 'header' ||
          tok.name === 'opaqueTag' || tok.name === 'blockTag' || (opts.inList && isList(tok))) {
        break;
      }

      node.contents += wsChunk;
      ...
      if (tok.name === 'text' || isWhitespace(tok)) {
        node.contents += escapeHtml(tok.contents);
        this._t.next();
        continue;
      }
      ...
      if (node.contents.length === 0) {
        ...
        node.contents.forEach(function (childNode) {
          if (childNode.name === 'tag') {
            childNode.name = 'text';
            childNode.contents = escapeHtml(childNode.contents);
          }
        });
